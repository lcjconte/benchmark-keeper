...
def add_run(run: BenchmarkRun):
    if not isinstance((runs := read_runs()), Report):
        raise RuntimeError(f"{REPORT_FILE} file badly formatted. Fix or delete it to continue.")
...
def get_commit_run(commit_id: str, experiment: str, experiment_version: int) -> BenchmarkRun | DataRetrieveFailure:
    proc = subprocess.run(
...
def get_current_run(experiment: str, experiment_version: int) -> BenchmarkRun | DataRetrieveFailure:
